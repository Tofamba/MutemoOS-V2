...
# ── TEMPORARY corpus-snapshot gate check (2026-09-23, re-added post-triage) ──
# Same as the earlier temporary gate-check endpoint (added, used, and
# removed once already this session) -- re-added to re-verify the gate
# after deleting the 11 known-broken rows found by the triage check above.
# No request body, no caller-supplied input of any kind -- runs
# scripts/corpus_snapshot.py publish --dry-run as a subprocess against
# THIS deployment's own DATABASE_URL/CHROMA_DATA_DIR only. Uploads nothing
# to R2. Gated by require_admin_token(), the same already-live check every
# other /api/admin/* endpoint uses. To be REMOVED once the gate is
# confirmed clean and the real publish has run.
