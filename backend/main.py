...
# TEMP (2026-09-03) -- real-data verification for Part C (compliance
# event logging), per instruction to verify a real change logs correctly
# with old/new values, and a no-op re-apply logs nothing new, before
# production. Uses _log_compliance_event() directly (the same helper the
# real endpoints call) rather than re-deriving the logic -- this checks
# the DB round-trip and _fetch_compliance_history()'s read side, which
# the unit tests (mocked DB) can't. X-Admin-Token gated. Removed once
# verified.
@app.get("/api/admin/verify-compliance-event-logging")
async def _verify_compliance_event_logging_TEMP(request: Request):
    require_admin_token(request)
    fake_user = {"id": None, "firm_id": FIRM_ID, "role": "admin", "display_name": "TEMP-VERIFY"}
    async with _db_pool.acquire() as conn:
        client_row = await conn.fetchrow("SELECT id FROM clients WHERE firm_id=$1 LIMIT 1", FIRM_ID)
        matter_row = await conn.fetchrow(
            "SELECT id FROM matters WHERE firm_id=$1 AND NOT is_sentinel LIMIT 1", FIRM_ID
        )
        cid, mid = client_row["id"], matter_row["id"]

        # Real change: client aml_scope
        existing = await conn.fetchrow(
            "SELECT aml_scope FROM client_compliance WHERE client_id=$1 AND firm_id=$2", cid, FIRM_ID
        )
        old_scope = (existing["aml_scope"] if existing else None) or "NotAssessed"
        new_scope = "OutOfScope" if old_scope != "OutOfScope" else "InScope"
        if existing:
            await conn.execute(
                "UPDATE client_compliance SET aml_scope=$1 WHERE client_id=$2 AND firm_id=$3", new_scope, cid, FIRM_ID
            )
        else:
            await conn.execute(
                "INSERT INTO client_compliance (client_id, firm_id, aml_scope) VALUES ($1,$2,$3)",
                cid, FIRM_ID, new_scope
            )
        await _log_compliance_event(conn, fake_user, "CLIENT", cid, "AML_SCOPE_SET", {"old": old_scope, "new": new_scope})

        # No-op: re-apply the SAME value -- real code's before/after check
        # would see new==old and skip the log call entirely, so nothing
        # more is logged here on purpose (this simulates that skip).
        await conn.execute(
            "UPDATE client_compliance SET aml_scope=$1 WHERE client_id=$2 AND firm_id=$3", new_scope, cid, FIRM_ID
        )

        # Real change: matter matter_risk
        before_matter = await conn.fetchrow("SELECT matter_risk FROM matters WHERE id=$1 AND firm_id=$2", mid, FIRM_ID)
        old_risk = before_matter["matter_risk"] or "NotAssessed"
        new_risk = "High" if old_risk != "High" else "Medium"
        await conn.execute("UPDATE matters SET matter_risk=$1 WHERE id=$2 AND firm_id=$3", new_risk, mid, FIRM_ID)
        await _log_compliance_event(conn, fake_user, "MATTER", mid, "MATTER_RISK_SET", {"old": old_risk, "new": new_risk})

        logs = await conn.fetch(
            "SELECT action, target_type, target_id, details, actor_name, created_at FROM audit_logs "
            "WHERE firm_id=$1 AND ((target_type='CLIENT' AND target_id=$2) OR (target_type='MATTER' AND target_id=$3)) "
            "ORDER BY created_at DESC LIMIT 10",
            FIRM_ID, cid, mid
        )
        aml_scope_set_count = sum(1 for l in logs if l["action"] == "AML_SCOPE_SET")
        matter_risk_set_count = sum(1 for l in logs if l["action"] == "MATTER_RISK_SET")

    return {
        "client_id": str(cid), "matter_id": str(mid),
        "client_change_applied": {"old": old_scope, "new": new_scope},
        "matter_change_applied": {"old": old_risk, "new": new_risk},
        "recent_logs": [
            {**dict(l), "target_id": str(l["target_id"]), "created_at": l["created_at"].isoformat(),
             "details": json.loads(l["details"]) if isinstance(l["details"], str) else l["details"]}
            for l in logs
        ],
        "aml_scope_set_logged_exactly_once_despite_noop_repatch": aml_scope_set_count == 1,
        "matter_risk_set_logged_exactly_once": matter_risk_set_count == 1,
    }
