...
@app.get("/api/admin/verify-matter-aml-pdf-wrap")
async def _TEMP_verify_matter_aml_pdf_wrap(request: Request):
    """
    TEMP, read-only, admin-token-gated -- real-data verification for the
    2026-09-07 Reason-column truncation fix. Regenerates the REAL PDF and
    CSV for this report (same helpers the real endpoints use), extracts
    the PDF's actual rendered text via pdfplumber, and reports, for every
    real matter whose aml_scope_reason is long enough that the old
    21%/single-line layout would have truncated it: whether the reason's
    start and end both survive intact in the rendered PDF, and whether
    the full reason survives verbatim in the CSV. Removed once verified.
    """
    require_admin_token(request)
    async with _db_pool.acquire() as conn:
        rows = await _fetch_matter_aml_status_rows(conn)

    long_rows = [r for r in rows if len(r["aml_scope_reason"]) > 60]

    # Real CSV, byte-for-byte what GET .../matter-aml-status-export returns.
    import csv, io as _io
    ...
    # Real PDF, byte-for-byte what GET .../matter-aml-status-export-pdf
    # returns -- same construction as that endpoint, inlined here since
    # this is a diagnostic reusing its helpers, not calling it as a route.
    from fpdf import FPDF
    ...
    with pdfplumber.open(_io.BytesIO(pdf_bytes)) as p:
        pdf_text = " ".join(" ".join(page.extract_text() or "" for page in p.pages).split())

    results = []
    for r in long_rows:
        reason = r["aml_scope_reason"]
        # Head/tail rather than the full string in one go: pdfplumber's
        # flat-text extraction places words by page position, so the
        # short Matter Status value sharing the row's first visual line
        # gets interleaved right after it -- a real artifact of reading a
        # multi-column table as flat text, not of the PDF's actual
    ...
    return {
        ...
        "long_reason_matters_checked": len(long_rows),
        "results": results,
    }
