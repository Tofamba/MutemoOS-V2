...
@app.get("/api/admin/verify-risk-rating-pdf-colors")
async def _TEMP_verify_risk_rating_pdf_colors(request: Request):
    """
    TEMP, read-only, admin-token-gated -- real-data verification that
    Risk Rating cell shading (commit bb4a78f) renders correctly in all
    three PDF exports against real firm data. Calls the exact same
    _build_*_pdf() functions the real endpoints call (not a
    reimplementation), fetched with the exact same fetch functions,
    then decodes each real PDF's own filled-rect colors via pdfplumber
    and compares the resulting multiset against what
    _pdf_color_for_risk_rating() should produce for each real client/
    matter's own risk_rating/matter_risk value. Removed once verified.
    """
    require_admin_token(request)
    import pdfplumber, io as _io

    def colors_close(a, b, tol=0.01):
        return all(abs(x - y) < tol for x, y in zip(a, b))

    def filled_rect_colors(pdf_bytes, exclude_color):
        colors = []
        with pdfplumber.open(_io.BytesIO(pdf_bytes)) as pdf:
            for page in pdf.pages:
                for rect in page.rects:
                    ...
                    if colors_close(color, exclude_color):
                        continue
                    colors.append(color)
        return colors

    def rgb_255_to_float(rgb):
        return tuple(c / 255 for c in rgb)

    navy_bg = rgb_255_to_float(_MP_PDF_COLORS["navy_bg"])

    async with _db_pool.acquire() as conn:
        ccs_rows = await _fetch_client_compliance_roster_rows(conn)
        matter_rows = await _fetch_matter_aml_status_rows(conn)
        ...
        cdd_reports = [await _fetch_client_aml_cdd_report(conn, c["id"], system_actor) for c in client_rows]

    ccs_pdf = _build_client_compliance_status_pdf(ccs_rows)
    ccs_colors = filled_rect_colors(ccs_pdf, navy_bg)
    ccs_expected = [rgb_255_to_float(_pdf_color_for_risk_rating(r["risk_rating"])[0]) for r in ccs_rows]

    matter_pdf = _build_matter_aml_status_pdf(matter_rows)
    matter_colors = filled_rect_colors(matter_pdf, navy_bg)
    matter_expected = [rgb_255_to_float(_pdf_color_for_risk_rating(r["matter_risk"])[0]) for r in matter_rows]

    # CDD report: one PDF per real client, each contributing 0-1 colored
    # cell for "Client Risk", 0-1 for "Client Risk Rating" (both same
    # value -> same color), and 1 per real matter for "Matter Risk".
    cdd_colors = []
    cdd_expected = []
    for report in cdd_reports:
        pdf_bytes = _build_client_cdd_pdf(report)
        cdd_colors.extend(filled_rect_colors(pdf_bytes, navy_bg))
        risk_bg = rgb_255_to_float(_pdf_color_for_risk_rating(report["overall"]["risk_rating"])[0])
        cdd_expected.append(risk_bg)  # Section 1: Client Risk
        cdd_expected.append(risk_bg)  # Section 5: Client Risk Rating (same underlying value)
        for mt in report["matters"]:
            cdd_expected.append(rgb_255_to_float(_pdf_color_for_risk_rating(mt["matter_risk"])[0]))

    def multiset_matches(actual, expected, tol=0.01):
        remaining = list(actual)
        for exp in expected:
            match = next((c for c in remaining if colors_close(c, exp, tol)), None)
            if match is None:
                return False, f"expected color not found: {exp}; remaining actual: {remaining}"
            remaining.remove(match)
        if remaining:
            return False, f"unexpected extra colors: {remaining}"
        return True, "ok"

    ccs_ok, ccs_detail = multiset_matches(ccs_colors, ccs_expected)
    matter_ok, matter_detail = multiset_matches(matter_colors, matter_expected)
    cdd_ok, cdd_detail = multiset_matches(cdd_colors, cdd_expected)

    return {
        "register": {"clients_checked": len(ccs_rows), "colored_cells": len(ccs_colors), "match": ccs_ok, "detail": ccs_detail},
        "matter_aml_status": {"matters_checked": len(matter_rows), "colored_cells": len(matter_colors), "match": matter_ok, "detail": matter_detail},
        "cdd_report": {"clients_checked": len(cdd_reports), "colored_cells": len(cdd_colors), "match": cdd_ok, "detail": cdd_detail},
        "all_match": ccs_ok and matter_ok and cdd_ok,
    }
