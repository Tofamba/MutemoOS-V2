...
# TEMP (2026-09-03) -- real-DB integration check for the delete_zlr_entry/
# delete_matter orphaned-chunks fix, against the actual Postgres schema
# (not the mocked unit tests) -- inserts its own small throwaway rows,
# exercises the identical SQL the fixed endpoints run, confirms 0 chunks
# remain afterward, and cleans up unconditionally in a finally block.
# X-Admin-Token gated, same pattern as every other one-off verification
# endpoint this session -- removed once verified.
@app.get("/api/admin/verify-delete-chunks-fix")
async def _verify_delete_chunks_fix_TEMP(request: Request):
    require_admin_token(request)
    results = {}
    zlr_id = matter_id = None
    async with _db_pool.acquire() as conn:
        try:
            # --- ZLR entry scenario ---
            zlr_id = await conn.fetchval(
                "INSERT INTO zlr_entries (firm_id, case_name) VALUES ($1, $2) RETURNING id",
                FIRM_ID, "ZZZ_TEMP_VERIFY_DELETE_FIX"
            )
            await conn.execute(
                "INSERT INTO chunks (id, firm_id, document_id, chunk_source, text) VALUES ($1,$2,$3,$4,$5)",
                "zzz_temp_chunk_zlr_1", FIRM_ID, zlr_id, "zlr", "temp verification chunk"
            )
            chunk_rows = await conn.fetch(
                "SELECT id FROM chunks WHERE document_id=$1 AND firm_id=$2", zlr_id, FIRM_ID
            )
            zlr_result = await conn.execute(
                "DELETE FROM zlr_entries WHERE id=$1 AND firm_id=$2", zlr_id, FIRM_ID
            )
            if zlr_result != "DELETE 0":
                await conn.execute(
                    "DELETE FROM chunks WHERE document_id=$1 AND firm_id=$2", zlr_id, FIRM_ID
                )
            zlr_remaining = await conn.fetchval(
                "SELECT COUNT(*) FROM chunks WHERE document_id=$1 AND firm_id=$2", zlr_id, FIRM_ID
            )
            results["zlr"] = {
                "deleted": zlr_result, "chunk_ids_found_before_delete": [r["id"] for r in chunk_rows],
                "remaining_chunks_after_delete": zlr_remaining,
            }

            # --- Matter scenario ---
            matter_id = await conn.fetchval(
                "INSERT INTO matters (firm_id, name) VALUES ($1, $2) RETURNING id",
                FIRM_ID, "ZZZ_TEMP_VERIFY_DELETE_FIX_MATTER"
            )
            matter_id_str = str(matter_id)
            await conn.execute(
                "INSERT INTO chunks (id, firm_id, document_id, matter_id, chunk_source, text) "
                "VALUES ($1,$2,$3,$4,$5,$6)",
                "zzz_temp_chunk_matter_1", FIRM_ID, uuid.uuid4(), matter_id_str, "firm", "temp verification chunk"
            )
            chunk_rows2 = await conn.fetch(
                "SELECT id FROM chunks WHERE matter_id=$1 AND firm_id=$2", matter_id_str, FIRM_ID
            )
            matter_result = await conn.execute(
                "DELETE FROM matters WHERE id=$1 AND firm_id=$2", matter_id, FIRM_ID
            )
            if matter_result != "DELETE 0":
                await conn.execute(
                    "DELETE FROM chunks WHERE matter_id=$1 AND firm_id=$2", matter_id_str, FIRM_ID
                )
            matter_remaining = await conn.fetchval(
                "SELECT COUNT(*) FROM chunks WHERE matter_id=$1 AND firm_id=$2", matter_id_str, FIRM_ID
            )
            results["matter"] = {
                "deleted": matter_result, "chunk_ids_found_before_delete": [r["id"] for r in chunk_rows2],
                "remaining_chunks_after_delete": matter_remaining,
            }
        finally:
            # Belt-and-suspenders cleanup in case anything above raised
            # partway through -- never leave synthetic rows behind either way.
            if zlr_id:
                await conn.execute("DELETE FROM chunks WHERE document_id=$1 AND firm_id=$2", zlr_id, FIRM_ID)
                await conn.execute("DELETE FROM zlr_entries WHERE id=$1 AND firm_id=$2", zlr_id, FIRM_ID)
            if matter_id:
                await conn.execute("DELETE FROM chunks WHERE matter_id=$1 AND firm_id=$2", str(matter_id), FIRM_ID)
                await conn.execute("DELETE FROM matters WHERE id=$1 AND firm_id=$2", matter_id, FIRM_ID)
