...
# TEMP (2026-09-04) -- real-data verification for the CDD report CSV/PDF
# export and the new Matter AML Status report, per instruction to verify
# against real multi-matter clients (Anchorflow + a second client, 4
# matters total) before production. Calls the real fetch/build functions
# directly (same pattern as every other one-off verification endpoint
# this session), bypassing the client:read/reports:* permission checks
# since this is read/write only through X-Admin-Token. Removed once
# verified.
@app.get("/api/admin/verify-cdd-export-and-matter-report")
async def _verify_new_aml_features_TEMP(request: Request):
    require_admin_token(request)
    results = {}
    async with _db_pool.acquire() as conn:
        anchorflow = await conn.fetchrow(
            "SELECT id, full_name FROM clients WHERE firm_id=$1 AND full_name ILIKE 'Anchorflow%' LIMIT 1", FIRM_ID
        )
        second = None
        if anchorflow:
            second = await conn.fetchrow(
                "SELECT client_id FROM matters WHERE firm_id=$1 AND NOT is_sentinel "
                "AND client_id IS NOT NULL AND client_id != $2 "
                "GROUP BY client_id HAVING COUNT(*) >= 2 ORDER BY COUNT(*) DESC LIMIT 1",
                FIRM_ID, anchorflow["id"]
            )

        if anchorflow:
            aid = anchorflow["id"]
            am_matters = await conn.fetch(
                "SELECT id, name FROM matters WHERE client_id=$1 AND firm_id=$2 AND NOT is_sentinel "
                "ORDER BY created_at ASC LIMIT 2",
                aid, FIRM_ID
            )
            if len(am_matters) >= 2:
                await conn.execute(
                    "UPDATE matters SET aml_scope='InScope', matter_risk='High', aml_scope_reason=$2 WHERE id=$1",
                    am_matters[0]["id"], "TEMP VERIFY: high-value transaction"
                )
                await conn.execute(
                    "UPDATE matters SET aml_scope='OutOfScope', matter_risk='Low' WHERE id=$1", am_matters[1]["id"]
                )

            report = await _fetch_client_aml_cdd_report(conn, aid)
            csv_text = _client_cdd_report_csv(report)
            pdf_bytes = _build_client_cdd_pdf(report)
            results["cdd_export"] = {
                "client": report["client"]["full_name"],
                "matters_in_report": [
                    {"name": mt["name"], "aml_scope": mt["aml_scope"], "matter_risk": mt["matter_risk"]}
                    for mt in report["matters"]
                ],
                "csv_length": len(csv_text),
                "csv_has_all_9_sections": all(
                    f"{i}." in csv_text for i in range(1, 10)
                ),
                "csv_matters_section_excerpt": next(
                    (line for line in csv_text.splitlines() if "InScope" in line or "OutOfScope" in line), None
                ),
                "pdf_is_valid_pdf": pdf_bytes[:4] == b"%PDF",
                "pdf_size_bytes": len(pdf_bytes),
            }
        else:
            results["cdd_export"] = {"error": "Anchorflow Holdings not found on this environment"}

        if second:
            sid = second["client_id"]
            second_client = await conn.fetchrow("SELECT full_name FROM clients WHERE id=$1 AND firm_id=$2", sid, FIRM_ID)
            sc_matters = await conn.fetch(
                "SELECT id, name FROM matters WHERE client_id=$1 AND firm_id=$2 AND NOT is_sentinel "
                "ORDER BY created_at ASC LIMIT 2",
                sid, FIRM_ID
            )
            if len(sc_matters) >= 2:
                await conn.execute(
                    "UPDATE matters SET aml_scope='InScope', matter_risk='Medium' WHERE id=$1", sc_matters[0]["id"]
                )
                await conn.execute(
                    "UPDATE matters SET aml_scope='NotAssessed', matter_risk='NotAssessed' WHERE id=$1",
                    sc_matters[1]["id"]
                )
            results["second_client"] = second_client["full_name"] if second_client else str(sid)
        else:
            results["second_client"] = None

        matter_rows = await _fetch_matter_aml_status_rows(conn)

    results["matter_aml_status_report"] = {
        "total_rows": len(matter_rows),
        "rows_for_anchorflow_and_second_client": [
            r for r in matter_rows
            if r["client_name"] in (
                anchorflow["full_name"] if anchorflow else None,
                results.get("second_client"),
            )
        ],
    }
    return results
