...
# TEMPORARY — one-time real-data verification for the Matter Review Status
# report (2026-08-30). AUTH_ENABLED is now True on staging (Africa's
# Talking configured), so a plain unauthenticated curl 401s; this reuses
# the session's established X-Admin-Token diagnostic pattern instead.
# Cross-checks the real report function's output against independently-
# computed raw values (deliberately NOT by calling _fetch_matter_review_status_rows
# twice and comparing it to itself -- that would just prove the function is
# consistent with itself, not correct against the DB) for up to `sample`
# real matters. Remove once staging verification is done.
@app.get("/api/admin/verify-matter-review-status")
async def admin_verify_matter_review_status(request: Request, sample: int = 20):
    require_admin_token(request)
    async with _db_pool.acquire() as conn:
        report_rows = await _fetch_matter_review_status_rows(
            conn, lawyer_id=None, client_id=None, status=None
        )
        report_rows = report_rows[:sample]

        mismatches = []
        checked = []
        for r in report_rows:
            mid = _uuid_mod.UUID(r["matter_id"])
            raw_matter = await conn.fetchrow(
                "SELECT next_review_date, last_reviewed_date, last_activity, created_at "
                "FROM matters WHERE id=$1", mid
            )
            ...
            expected_last_reviewed = raw_matter["last_reviewed_date"].isoformat() if raw_matter["last_reviewed_date"] else None
            if raw_note:
                expected_kind = "note"
                expected_date = raw_note["created_at"].isoformat()
            elif raw_doc:
                expected_kind = "document"
                expected_date = raw_doc["uploaded_at"].isoformat()
            elif raw_matter["last_activity"]:
                expected_kind = "touched"
                expected_date = raw_matter["last_activity"].isoformat()
            else:
                expected_kind = "created"
                expected_date = raw_matter["created_at"].isoformat()

            row_mismatches = []
            if r["next_review_date"] != expected_next_review:
                row_mismatches.append(f"next_review_date: report={r['next_review_date']} raw={expected_next_review}")
            if r["last_reviewed_date"] != expected_last_reviewed:
                row_mismatches.append(f"last_reviewed_date: report={r['last_reviewed_date']} raw={expected_last_reviewed}")
            if r["last_activity_kind"] != expected_kind:
            ...
            entry = {
                ...
                "report": {
                    "next_review_date": r["next_review_date"], "last_reviewed_date": r["last_reviewed_date"],
                    "last_activity_kind": r["last_activity_kind"], "last_activity_text": r["last_activity_text"],
                    "last_activity_date": r["last_activity_date"],
                },
                "raw": {
                    "next_review_date": expected_next_review, "last_reviewed_date": expected_last_reviewed,
                    "last_activity_kind": expected_kind, "last_activity_date": expected_date,
                    "most_recent_note_text": raw_note["text"] if raw_note else None,
                    "most_recent_doc_filename": raw_doc["filename"] if raw_doc else None,
                },
            }
            checked.append(entry)
            if row_mismatches:
                mismatches.append({"matter_id": r["matter_id"], "matter_name": r["matter_name"], "issues": row_mismatches})

    return {
        "total_matters_in_report": len(report_rows),
