...
# TEMPORARY -- URGENT, final confirmation for the MLPCA contradiction
# (2026-09-02): user supplied the literal query text used for the
# "sections 3-37C entirely absent" run -- a long natural-language
# meta-question ABOUT the corpus/retrieval system, not the literal
# statutory phrase. Runs that exact literal string through the real
# /api/search code path (_semantic_search_legal(), same Postgres query)
# to show precisely what it retrieves and why, confirming or refuting
# the query-embedding-mismatch hypothesis with hard evidence rather
# than inference. Read-only. Removed once confirmed.
@app.get("/api/admin/confirm-mlpca-query-mismatch")
async def admin_confirm_mlpca_query_mismatch(request: Request):
    require_admin_token(request)
    literal_meta_query = (
        "There's a direct contradiction that needs resolving before anything else. "
        "Multiple queries earlier this week confirmed sections 15, 16, 17, 18, 19, 20, "
        "and 24 of the Money Laundering and Proceeds of Crime Act ARE present in the "
        "corpus, with real citations and direct quotes retrieved. A query just now "
        "concluded sections 3 through 37C are \"entirely absent\" from the corpus. Both "
        "cannot be true. Resolve this directly: 1. Search again, right now, for section "
        "17's actual content — the specific text: \"for a customer who is an individual, "
        "his or her full name and date and place of birth\" (confirmed present multiple "
        "times this week). Is it findable right now, yes or no? 2. Search for section "
        "20's PEP content — \"shall have appropriate risk management systems\" and "
        "\"politically-exposed person\" (also confirmed present multiple times this "
        "week). Findable right now, yes or no?"
    )
    clean_phrase_query = "politically-exposed person"
    ...
    async def _run(query):
        ...
        return {
            "total_hits": len(hits),
            "mlpca_hits": sum(1 for h in hits if h["document_id"] in mlpca_doc_ids),
            "results": [
                {"document_id": h["document_id"], "source_name": h.get("source_name"),
                 "similarity": h["similarity"], "snippet": h["text"][:150]}
                for h in hits
            ],
        }

    return {
        "literal_meta_query_used_by_user": {
            "query_text": literal_meta_query,
            "result": await _run(literal_meta_query),
        },
        "clean_statutory_phrase_for_comparison": {
            "query_text": clean_phrase_query,
            "result": await _run(clean_phrase_query),
        },
    }
