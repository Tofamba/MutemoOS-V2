...
# Real corpus-snapshot publish, run in-process against THIS deployment's own
# DATABASE_URL/CHROMA_DATA_DIR (the same values get_chroma_collections() uses)
# -- do_export()/do_publish() need direct Postgres + Chroma-filesystem access
# that no external caller (including this session's own shell) can reach, so
# this runs the real script logic from inside the one process that already
# has it, exactly like every other real-data verification this week.
@app.post("/api/admin/corpus-snapshot-publish")
async def admin_corpus_snapshot_publish(request: Request):
    require_admin_token(request)
    dry_run = request.query_params.get("dry_run", "true").lower() != "false"
    from scripts import corpus_snapshot
    database_url = os.environ.get("DATABASE_URL")
    chroma_path = os.environ.get(
    ...
    try:
        with contextlib.redirect_stdout(buf):
            await corpus_snapshot.do_publish(database_url, chroma_path, str(FIRM_ID), dry_run=dry_run)
        return {"ok": True, "dry_run": dry_run, "output": buf.getvalue()}
    except SystemExit as e:
        return {"ok": False, "dry_run": dry_run, "exit_code": e.code, "output": buf.getvalue()}

# Real corpus-snapshot restore, into a genuinely separate throwaway
# Postgres+Chroma (never this deployment's own DATABASE_URL/CHROMA_DATA_DIR)
# -- the target database_url is supplied in the request body (a throwaway
# instance provisioned outside this app entirely), so do_import() never
# touches this deployment's real data. Chroma likewise writes to a fresh
# tempfile.mkdtemp() scratch directory, not CHROMA_DATA_DIR, cleaned up
# before returning. R2 download uses this deployment's own real credentials
# (the whole point -- no other caller has them), but that's a read of the
# already-published, already-public snapshot, same as any real onboarding
# restore would do.
@app.post("/api/admin/corpus-snapshot-restore-test")
async def admin_corpus_snapshot_restore_test(request: Request):
    require_admin_token(request)
    body = await request.json()
    target_database_url = body["database_url"]
    target_firm_id = body["firm_id"]
    snapshot = body.get("snapshot", "latest")
    query = body.get("query", "Constitution of Zimbabwe")

    from scripts import corpus_snapshot
    import tempfile, shutil as _shutil, io, contextlib
    import chromadb as _chromadb
    import uuid as _uuid_mod2

    chroma_scratch = tempfile.mkdtemp(prefix="corpus_restore_test_")
    buf = io.StringIO()
    try:
        with contextlib.redirect_stdout(buf):
            await corpus_snapshot.do_restore(
                target_database_url, chroma_scratch, target_firm_id, snapshot, apply=True
            )

        client = _chromadb.PersistentClient(path=chroma_scratch)
        query_vec = (await asyncio.to_thread(embed_texts, [query]))[0]
        chroma_results = {}
        for coll_name in ("legal_updates", "zlr_index"):
            try:
                col = client.get_collection(coll_name)
            except Exception:
                chroma_results[coll_name] = {"vector_count": 0, "hits": []}
                continue
            q = col.query(
            ...
            chroma_results[coll_name] = {"vector_count": col.count(), "hits": hits}

        conn = await asyncpg.connect(target_database_url)
        try:
            fid = _uuid_mod2.UUID(target_firm_id)
            pg_counts = {
                "legal_updates": await conn.fetchval("SELECT COUNT(*) FROM legal_updates WHERE firm_id=$1", fid),
                "zlr_entries": await conn.fetchval("SELECT COUNT(*) FROM zlr_entries WHERE firm_id=$1", fid),
                "chunks": await conn.fetchval("SELECT COUNT(*) FROM chunks WHERE firm_id=$1", fid),
            }
        finally:
            await conn.close()

        return {"postgres_counts": pg_counts, "chroma_search": chroma_results, "log": buf.getvalue()}
    finally:
        _shutil.rmtree(chroma_scratch, ignore_errors=True)
