...
@app.get("/api/admin/cf-access-app-info")
async def cf_access_app_info(request: Request):
    """TEMPORARY diagnostic endpoint (2026-08-27) -- not a standing feature.

    The user's Zero Trust dashboard was showing an empty/onboarding Access
    Applications list instead of the real app protecting mutemo.tofamba.com,
    even in the confirmed-correct account. Manual navigation wasn't finding
    it. CLOUDFLARE_ACCESS_APP_ID has been configured in production this
    whole time -- _add_email_to_cloudflare_access() above already depends on
    it being correct for every invite -- but nothing has ever surfaced what
    it actually points to. Rather than guess at a dashboard URL, this asks
    Cloudflare's own API directly, using the exact same account/app id/token
    the app already trusts for real work, and returns only the non-secret
    identifying fields plus a direct deep link.

    Safe to delete once this investigation is resolved: read-only (a plain
    GET against Cloudflare's Access Applications API, no state changed on
    either side), and gated by X-Admin-Token same as bootstrap_firm() below.
    """
    admin_token_header = request.headers.get("X-Admin-Token", "")
    ...
    missing = [
        name for name, val in [
            ...
            ("CLOUDFLARE_ACCESS_APP_ID", CLOUDFLARE_ACCESS_APP_ID),
        ] if not val
    ]
    if missing:
        return {"error": "Cloudflare vars not fully configured on this deployment", "missing": missing}

    try:
        import httpx
        async with httpx.AsyncClient(timeout=15) as http:
            resp = await http.get(
                f"https://api.cloudflare.com/client/v4/accounts/{CLOUDFLARE_ACCOUNT_ID}/access/apps/{CLOUDFLARE_ACCESS_APP_ID}",
                headers={"Authorization": f"Bearer {CLOUDFLARE_API_TOKEN}"},
            )
    except Exception as e:
    ...
    if resp.status_code != 200:
        return {
            "error": f"Cloudflare API returned {resp.status_code}",
            "body": resp.text[:500],
            "account_id": CLOUDFLARE_ACCOUNT_ID,
            "app_id": CLOUDFLARE_ACCESS_APP_ID,
        }
    ...
    return {
        ...
        "account_id": CLOUDFLARE_ACCOUNT_ID,
        "dashboard_url": f"https://one.dash.cloudflare.com/{CLOUDFLARE_ACCOUNT_ID}/access/apps/edit/{app_id}",
    }
