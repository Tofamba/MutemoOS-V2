...
| `AFRICAS_TALKING_USERNAME`, `AFRICAS_TALKING_API_KEY` | | OTP login delivery via Africa's Talking SMS — the **primary** SMS channel, checked ahead of Twilio (see Authentication note below) |
| `AFRICAS_TALKING_FROM` | | Optional registered alphanumeric sender ID for Africa's Talking. **Unset = AT's shared `AFRICASTKNG` sender**, which Zimbabwean carriers silently filter for a meaningful share of OTP traffic. Set this to a POTRAZ-registered sender ID once one is approved. |
| `AFRICAS_TALKING_DLR_TOKEN` | | Optional shared secret for the Africa's Talking delivery-report webhook (`POST /api/webhooks/africas-talking/delivery-report?token=…`). If set, callbacks without an exact match are rejected; if unset the endpoint still works. See Delivery reports note below. |
| `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_FROM_NUMBER` | | OTP login delivery via Twilio SMS (legacy fallback, never configured for this firm — see Authentication note below) |
...
> **Note on authentication:** OTP delivery tries four channels in order, each activating automatically as soon as its own env vars are set — no code change needed to switch: WhatsApp (`WHATSAPP_ACCESS_TOKEN`/`WHATSAPP_PHONE_NUMBER_ID`) first, then Africa's Talking SMS (`AFRICAS_TALKING_USERNAME`/`AFRICAS_TALKING_API_KEY`) as the primary SMS channel, then Twilio SMS (`TWILIO_ACCOUNT_SID`/`TWILIO_AUTH_TOKEN`/`TWILIO_FROM_NUMBER`) as a legacy fallback that has never actually been configured for this firm, then email via Resend as a last-resort stopgap. Account creation is invite-gated: a phone number must match a pending, unaccepted invite (or an existing active user) to receive a code at all — this replaced the previous static `MUTEMO_ALLOWED_PHONES` allowlist, which is no longer used. If **none** of WhatsApp, SMS, or email is configured, auth is disabled and the app falls back to a synthetic development user — this is a dev/demo fallback only and must not be relied on for a deployment holding real client data.

> **Note on SMS delivery reports:** Africa's Talking's send response only confirms the message was *accepted* (`statusCode 100`), not that it reached the handset — Zimbabwean carriers can silently drop OTP SMS with no error. `POST /api/webhooks/africas-talking/delivery-report` receives AT's asynchronous delivery report (final status: `Success` / `Failed` / `Rejected`, plus `failureReason`) and records it against the matching `sms_usage_log` row by `message_id`, so silent drops become visible in `GET /api/reports/sms-usage-by-firm` (`carrier_confirmed_count` / `carrier_failed_count` / `carrier_unconfirmed_count`). To activate: in the Africa's Talking dashboard, set the app's **Delivery Reports** callback URL to `https://<host>/api/webhooks/africas-talking/delivery-report` (append `?token=<AFRICAS_TALKING_DLR_TOKEN>` if that var is set). The AT account/app is shared across firm deployments and AT allows one callback URL per app — point it at production; the endpoint ignores delivery reports whose `message_id` it doesn't recognise, so a stray staging callback is a harmless no-op.
